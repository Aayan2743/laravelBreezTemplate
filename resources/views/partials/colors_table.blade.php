<div class="table-responsive">
    <table class="table">
        ...
        @else   
            ...
            <tr>
                <td>{{$key+1}}</td>
                <td>{{ $client->color_code ?? 'No Data' }}</td>
             
               
                 <td><label class="badge badge-gradient-warning"><a href="javascript:void(0);" 
                    data-bs-toggle="modal" 
                    data-color_id="{{$client->color_id}}" 
                    data-color_code="{{$client->color_code}}"
                   ...
                   <td>
                    <!-- <label class="badge badge-gradient-danger">
                        ...
                        onclick="confirmDelete('{{ route('color.destroy', $client->color_id ) }}')" 
                    ...
                    <form id="delete-form" method="POST" style="display: none;">
                        ...
                        <label class="badge badge-gradient-danger">
                        <a href="javascript:void(0);" onclick="confirmDelete('{{ route('color.destroy', $client->color_id) }}')" 
                        style="text-decoration: none;">Delete</a>
                    </label>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="coBrandingForm" action="{{ route('color.store') }}" method="POST" >
                @csrf
                   

                   
                    <div class="mb-3">
                        <label for="coBrandingText" class="form-label">Enter Color Code</label>
                        <input type="text" class="form-control" name="color_code" id="color_code" required placeholder="Enter Color code">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form id="coBrandingForm" action="{{ route('color.updateDetails') }}" method="POST">
                @csrf
              

                  
                    <div class="mb-3">
                        <label for="coBrandingText" class="form-label">Enter Color Name</label>
                        <input type="text" class="form-control"  name="color_codes" required id="color_codes" required placeholder="Enter Name">
                        <input type="hidden" class="form-control" name="color_id" readonly id="color_id" placeholder="Enter Co-branding Name">
                    </div>
        ...
        $('#coBrandingModal').on('show.bs.modal', function (event) {
            ...
            $('#clientNameDisplay').text(clientName); // If displaying in a <span>
             ...
             document.getElementById('editRateCard').addEventListener('show.bs.modal', function(event) {
                ...
                let color_id = button.getAttribute('data-color_id'); 
                let color_code = button.getAttribute('data-color_code'); 
              
              
                console.log(color_code);

                $('#color_id').val(color_id);
                $('#color_codes').val(color_code);
